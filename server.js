...
(async () => {
  ...
  // Route for the intermediate iframe "setget.hbs"
  server.get("/setget", async (request, reply) => {
    reply.header('Content-Type', 'text/html'); 
    setViewParams(request, 'setget');
    tryForwardScriptAsText("public/setget-module.js");
    return await reply.view("/src/pages/setget.hbs", viewParams);
  });
  
  // Route for our JS module with setget function
  server.get("/setget-module.js", async (request, reply) => {  
    tryForwardScript(reply, "public/setget-module.js");  
  });

  // Route for the fenced frame created by script
  server.get('/setget-inner', async (request, reply) => {
    setFencedFrameHeaders(reply);
    setViewParams(request, 'setget');
    return await reply.view("/src/pages/setget-inner.hbs", viewParams);
  });
